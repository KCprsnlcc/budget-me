...
import { createAdminBudget } from "../_lib/admin-budget-service";
import { UserAvatar } from "@/components/shared/user-avatar";
import type { User as SupabaseUser } from "@supabase/supabase-js";

interface AddAdminBudgetModalProps {
    open: boolean;
    onClose: () => void;
    onSuccess: () => void;
}
...
export function AddAdminBudgetModal({ open, onClose, onSuccess }: AddAdminBudgetModalProps) {
    ...
    const endOfMonth = `${lastDayOfMonth.getFullYear()}-${String(lastDayOfMonth.getMonth() + 1).padStart(2, "0")}-${String(lastDayOfMonth.getDate()).padStart(2, "0")}`;
    ...
    const handleSubmit = async () => {
        try {
            setLoading(true);
            const { error } = await createAdminBudget(formData.user_id, {
                    ...
                    <div className="animate-txn-in">
                        <div className="mb-5">
                            ...
                            <p className="text-[11px] text-gray-500">
                                Choose the user to create a budget for.
                            </p>
                        </div>
                            ...
                            <div className="flex gap-2.5 p-3 rounded-lg text-xs bg-white border border-slate-200 text-slate-700 items-start">
                                ...
                                <div>
                                    ...
                                    <p className="text-[11px] leading-relaxed">
                                        The budget will be created immediately and added to the user's account.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <ModalFooter className="flex justify-between">
                ...
                {currentStep < STEPS.length ? (
                    ...
                    <Button size="sm" onClick={handleSubmit} disabled={loading} className="bg-emerald-500 hover:bg-emerald-600">
                        {loading ? (<><Loader2 size={14} className="animate-spin" /> Creating...</>) : (<>Create Budget <Check size={14} /></>)}
                    </Button>
                )}
            </ModalFooter>
        </Modal>

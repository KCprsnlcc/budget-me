    ...
    <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6 animate-fade-in h-full flex flex-col overflow-hidden lg:overflow-visible">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 px-4 sm:px-0 pt-4 sm:pt-0 shrink-0">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1 font-light">View and manage all your income and expense transactions.</p>
        </div>
        <div className="flex gap-2 sm:gap-3 flex-wrap sm:flex-nowrap">
          ...
          <Button size="sm" className="bg-emerald-500 hover:bg-emerald-600 order-2 w-full sm:w-auto" onClick={() => setAddModalOpen(true)}>
            <Plus size={14} className="sm:mr-1" /> <span className="hidden sm:inline">Add Transaction</span><span className="sm:hidden">Add</span>
          </Button>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex p-1 bg-slate-100 rounded-lg lg:hidden mb-4">
          <Button
            ...
            className={`flex-1 text-xs transition-colors ${
              mobileChartTab === 'income' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          ...
          <Button
            ...
            size="sm"
            className={`flex-1 text-xs transition-colors ${
              mobileChartTab === 'categories' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
        ...
        <Card className={`lg:col-span-2 p-4 sm:p-6 hover:shadow-md transition-all group cursor-pointer ${mobileChartTab === 'categories' ? 'hidden lg:block' : ''}`}>
          <div className="flex items-center justify-between mb-6 sm:mb-8">
            <div>
              <h3 className="text-xs sm:text-sm font-semibold text-slate-900">Income vs Expenses</h3>
            ...
            <div className="flex gap-2 sm:gap-3">
              <div className="flex items-center gap-1 sm:gap-1.5">
                ...
                <span className="text-[9px] sm:text-[10px] font-medium text-slate-500">Income</span>
              </div>
              <div className="flex items-center gap-1 sm:gap-1.5">
                <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-500" />
                <span className="text-[9px] sm:text-[10px] font-medium text-slate-500">Expense</span>
              </div>
            </div>
          </div>
          ...
          {chartData.length > 0 ? (
              <>
                <div className="relative h-48 sm:h-60 flex items-end justify-between gap-1 sm:gap-6 px-2 border-b border-slate-50">
                  ...
                  {chartData.map((d) => (
                    <div key={d.month} className="flex gap-0.5 sm:gap-1 h-full items-end flex-1 justify-center z-10 group cursor-pointer relative">
                      <div
                      ...
                      {hoveredBar && hoveredBar.month === d.month && (
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-white border border-slate-200 text-slate-900 text-[10px] sm:text-xs rounded shadow-sm whitespace-nowrap z-50">
                          ...
                          <div className="flex items-center gap-1">
                            <div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${hoveredBar.type === 'income' ? 'bg-slate-300' : 'bg-emerald-500'}`} />
        ...
        <Card className={`p-4 sm:p-6 flex flex-col hover:shadow-md transition-all group cursor-pointer ${mobileChartTab === 'income' ? 'hidden lg:flex' : ''}`}>
          <div className="mb-4 sm:mb-6">
            ...
            <p className="text-[10px] sm:text-xs text-slate-500 mt-1 font-light">Monthly expense breakdown.</p>
          </div>
              ...
              <>
                <div className="flex items-center gap-4 sm:gap-6 mb-4 sm:mb-6">
                  <div className="w-24 h-24 sm:w-32 sm:h-32 mx-auto rounded-full flex-shrink-0 relative"
                    ...
                    <div className="absolute inset-0 m-auto w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-full flex flex-col items-center justify-center shadow-sm">
                      ...
                      <span className="text-sm sm:text-xl font-bold text-slate-900">{formatCompact(categoryTotal)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2 sm:space-y-3 flex-1">
                  {categoryBreakdown.slice(0, 5).map((cat) => (
                    <div key={cat.name} className="flex items-center justify-between text-[10px] sm:text-xs">
      ...
      <Card className="p-3 sm:p-4 hover:shadow-md transition-all group cursor-pointer">
        <div className="flex flex-col xl:flex-row items-center gap-2 sm:gap-3">
          ...
          <div className="grid grid-cols-2 xl:flex items-center gap-2 w-full xl:w-auto">
            <FilterDropdown
